...
async function broadcastUserList() {
    try {
        const result = await pool.query('SELECT id, name, avatar, online FROM users ORDER BY name');
...
// Отправка истории сообщений конкретному пользователю
        ...
        try {
            ...
            if (msg.type === 'register') {
                ...
                if (exists.rows.length === 0) {
                    await pool.query(
                        'INSERT INTO users (id, name, avatar, online) VALUES ($1, $2, $3, $4)',
                        [userId, msg.name, msg.avatar || '😊', true]
                    );
                } else {
                ...
                console.log(`✅ Пользователь ${msg.name} подключился`);
            }
            ...
            if (msg.type === 'message') {
                ...
                console.log(`📨 Сообщение от ${msg.fromName} к ${msg.to}`);
            }
